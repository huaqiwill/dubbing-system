...
async function uploadFile(localFilePath, remoteFilePath) {
  try {
    ...
    console.log(
      `文件上传成功：从本地 ${localFilePath} 上传到 ${remoteFilePath}`
    );
  } catch (error) {
...
async function syncDatabase() {
  ...
  await execCommand(
    `docker exec -i mysql mysql -u${mysqlUser} -p${mysqlPassword} -e "drop database if exists ${database};create database if not exists ${database};"`
  );
  await execCommand(
    `docker exec -i mysql mysql -u${mysqlUser} -p${mysqlPassword} --force ${database} < ${remoteFilePath}`
  ).catch((err) => {});
...
async function syncUser() {
  ...
  await execCommand(
    "unzip -o /usr/share/nginx/html/user.zip -d /usr/share/nginx/html/user"
  );
...
async function syncAdmin() {
  ...
  await execCommand(
    "unzip -o /usr/share/nginx/html/admin.zip -d /usr/share/nginx/html/admin"
  );
...
async function startJavaServerProcess() {
  ...
  await execCommand(
    "cd /root/server && nohup /root/java/bin/java -jar ./moyin-admin.jar -Dspring.config.location=./application.yml > running.log 2>&1 &"
  );
...
async function killJavaServerProcess() {
  try {
    const searchCommand =
      "ps aux | grep 'moyin-admin.jar' | grep -v grep | awk '{print $2}'";
