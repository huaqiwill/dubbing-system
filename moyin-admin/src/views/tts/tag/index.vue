<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="标签值" prop="name">
        <el-input
          v-model="queryParams.name"
          placeholder="请输入标签名称"
          clearable
    ...
    <pagination
      v-show="total > 0"
      :total="total"
      :pTag.sync="queryParams.pTagNum"
      :limit.sync="queryParams.pTagSize"
      @pagination="getList"
    />
...
<script>
import { listTag, getTag, delTag, addTag, updateTag } from "@/api/tts/tag";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除配音员标签编号为"' + ids + '"的数据项？')
        .then(function () {
          return delTag(ids);
        })
        .then(() => {
